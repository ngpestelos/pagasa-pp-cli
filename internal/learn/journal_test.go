...
func withJournalHome(t *testing.T) string {
	...
	t.Setenv("PAGASA_NO_LEARN", "")
	t.Setenv("PAGASA_LEARN_NO_CAPTURE", "")
	t.Setenv("PAGASA_LEARN_SESSION", "")
	return dir
}
...
func runCLI(t *testing.T, args ...string) error {
	t.Helper()
	oldArgs := os.Args
	os.Args = append([]string{"pagasa-pp-cli"}, args...)
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("PAGASA_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "master no-learn env", env: map[string]string{"PAGASA_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"PAGASA_LEARN_NO_CAPTURE": "1"}},
