...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	httpClient := newHTTPClient(timeout, nil, cfg.SkipTLSVerify)
	c := &Client{
...
func (c *Client) GetWithHeaders(ctx context.Context, path string, params map[string]string, headers map[string]string) (json.RawMessage, error) {
	...
	if cacheEnabled {
		if cached, contentType, ok := c.readCacheWithHeaders(path, params, headers); ok {
	...
	if err == nil && cacheEnabled {
		c.writeCacheWithHeaders(path, params, headers, result, c.lastContentType)
	}
	return result, err
}
...
func (c *Client) readCacheWithHeaders(path string, params map[string]string, headers map[string]string) (json.RawMessage, string, bool) {
...
func (c *Client) writeCache(path string, params map[string]string, data json.RawMessage, contentType string) {
	c.writeCacheWithHeaders(path, params, nil, data, contentType)
}
