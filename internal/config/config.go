...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("PAGASA_SKIP_TLS_VERIFY"))); v == "1" || v == "true" || v == "yes" || v == "on" {
		cfg.SetSkipTLSVerify(true)
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("PAGASA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PAGASA_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "pagasa-pp-cli", "config.toml"), nil
}
