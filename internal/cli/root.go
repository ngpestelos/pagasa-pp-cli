...
type rootFlags struct {
	...
	// invocation. Mirrors the PAGASA_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pagasa-pp-cli",
		Short: `Pagasa CLI — Every PAGASA public weather surface as one agent-native CLI — synopsis, city forecast, and live cyclone signals, with…`,
		Long: `Pagasa CLI — Every PAGASA public weather surface as one agent-native CLI — synopsis, city forecast, and live cyclone signals, with a local history no PAGASA page keeps.
...
Health check: run 'pagasa-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pagasa-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'pagasa-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNowCmd(flags))
	rootCmd.AddCommand(newForecastCmd(flags))
	rootCmd.AddCommand(newStormCmd(flags))
	rootCmd.AddCommand(newNovelApproachCmd(flags))
	...
	rootCmd.AddCommand(newTropicalCyclonePromotedCmd(flags))
