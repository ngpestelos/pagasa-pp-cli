...
func emitSyncHints(w io.Writer, db *store.Store, resourceType string, maxAge time.Duration) {
	...
	if !state.hasState {
		fmt.Fprintf(w, "hint: local store has not been synced yet. Run 'pagasa-pp-cli sync' before trusting local results.\n")
		return
	}
	if maxAge <= 0 {
		return
	...
	fmt.Fprintf(w, "hint: local store data is %s old, older than --max-age=%s. Run 'pagasa-pp-cli sync' to refresh.\n", syncHintRoundAge(age), maxAge)
}
...
func hintIfUnsynced(cmd *cobra.Command, db *store.Store, resourceType string) bool {
	...
	fmt.Fprintf(cmd.ErrOrStderr(), "hint: local store has not been synced yet. Run 'pagasa-pp-cli sync' before trusting local results.\n")
	return true
}
...
func hintIfStale(cmd *cobra.Command, db *store.Store, resourceType string, maxAge time.Duration) bool {
	...
	fmt.Fprintf(cmd.ErrOrStderr(), "hint: local store data is %s old, older than --max-age=%s. Run 'pagasa-pp-cli sync' to refresh.\n", syncHintRoundAge(age), maxAge)
	return true
}
