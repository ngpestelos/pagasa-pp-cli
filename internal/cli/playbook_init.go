// Copyright 2026 ngpestelos. Licensed under Apache-2.0. See LICENSE.
...
import (
	...
	"github.com/ngpestelos/pagasa-pp-cli/internal/cli/playbooks"
...
func runPlaybookInitOnce(ctx context.Context) {
	playbookInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := installPlaybooksFromEmbed(ctx, s, playbooks.FS); err != nil {
			fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: %v\n", err)
		}
	})
}
...
func installPlaybooksFromEmbed(ctx context.Context, s *store.Store, embedFS fs.FS) error {
	...
	for _, base := range bases {
		...
		if rerr != nil {
			fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: read %s: %v\n", jsonName, rerr)
			upsertFailed = true
			continue
		}
		pb, perr := learn.ParsePlaybook(jsonData, jsonName)
		if perr != nil {
			fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: parse %s: %v\n", jsonName, perr)
			upsertFailed = true
			continue
		}
		...
		if len(families) == 0 {
			...
			fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: %s has no query_family_examples; skipping (would be unreachable at recall time)\n", jsonName)
			continue
		}
		...
		if merr != nil {
			fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: marshal %s: %v\n", jsonName, merr)
			upsertFailed = true
			continue
		}
		...
		for _, family := range famList {
			...
			if _, _, uerr := s.UpsertPlaybook(store.UpsertPlaybookInput{
				...
				fmt.Fprintf(os.Stderr, "warning: pagasa-pp-cli: playbook init: upsert family=%q for %s: %v\n", family, jsonName, uerr)
				upsertFailed = true
				continue
			}
		}
