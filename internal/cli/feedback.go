...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAGASA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAGASA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pagasa-pp-cli/feedback")
...
When ` + "`PAGASA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAGASA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pagasa-pp-cli feedback list
  pagasa-pp-cli feedback list --limit 5
  pagasa-pp-cli feedback list --json`,
