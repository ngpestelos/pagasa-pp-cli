...
func legacyProfileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".pagasa-pp-cli")
	return filepath.Join(dir, "profiles.json"), nil
}
...
captured: they control profile/config resolution and would never apply).`,
		Example: `  pagasa-pp-cli profile save my-defaults --json --compact
  pagasa-pp-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  pagasa-pp-cli profile use my-defaults
  pagasa-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  pagasa-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  pagasa-pp-cli profile show my-defaults
  pagasa-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  pagasa-pp-cli profile delete my-defaults --yes
  pagasa-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
