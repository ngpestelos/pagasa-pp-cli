...
// pagasa-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "PAGASA_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("pagasa-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  pagasa-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  pagasa-pp-cli recall "<question>" --agent
  pagasa-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pagasa-pp-cli learnings list --agent
  pagasa-pp-cli learnings list --query "<substring>"
  pagasa-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  pagasa-pp-cli learnings forget "<question>" --resource <id>
  pagasa-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  pagasa-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  pagasa-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
