...
func newSyncHintTestCmd() (*cobra.Command, *bytes.Buffer) {
	...
	cmd := &cobra.Command{Use: "pagasa-pp-cli"}
	cmd.SetErr(&stderr)
	return cmd, &stderr
}
...
func TestHintIfUnsynced_EmptySyncStateWritesHintToStderr(t *testing.T) {
	if !syncHintsEnabled {
		t.Skip("sync hints are compile-time disabled in this build")
	}
	...
	if got := stderr.String(); !strings.Contains(got, "Run 'pagasa-pp-cli sync'") {
...
func TestHintIfStale_BackdatedSyncStateWritesHintToStderr(t *testing.T) {
	...
	got := stderr.String()
	if !strings.Contains(got, "older than --max-age=30m0s") || !strings.Contains(got, "Run 'pagasa-pp-cli sync'") {
