...
alter command behavior or recall's verified results on their own.`,
		Example: `  pagasa-pp-cli learnings candidates
  pagasa-pp-cli learnings candidates --class flag_alias
  pagasa-pp-cli learnings candidates --status rejected`,
...
Usage errors (unknown id, non-open status) exit 2.`,
		Example: `  pagasa-pp-cli learnings confirm 3`,
...
Usage errors (unknown id, no-reject-path candidates) exit 2.`,
		Example: `  pagasa-pp-cli learnings reject 3`,
		// Usage errors here exit 2 by design; declare it so
		// `cli-printing-press verify` scores the Execute cell honestly.
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if errors.Is(err, store.ErrConfirmedFlagAliasReject) {
				return usageErr(fmt.Errorf("learnings reject: candidate %d is a confirmed flag correction with no reject path; repair the guidance with \"pagasa-pp-cli playbook amend\" instead", id))
			}
			if err != nil {
				return usageErr(fmt.Errorf("learnings reject: %w", err))
			}
...
purge also clears stale candidates nobody judged.`,
		Example: `  pagasa-pp-cli learnings purge
  pagasa-pp-cli learnings purge --tombstones`,
