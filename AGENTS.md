# PAGASA Printed CLI Agent Guide

This directory is a generated `pagasa-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pagasa-pp-cli doctor --json
pagasa-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pagasa-pp-cli which "<capability>" --json
pagasa-pp-cli <command> --help
```
...
```bash
pagasa-pp-cli <command> --help
pagasa-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `pagasa-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Annotations: `recall`, `learnings list`, `learnings candidates`, and `learnings stats` carry `mcp:read-only=true` (local → walker sets `openWorldHint=false`); live scrapers (`forecast`, `storm`, `watch`, `approach`, `weather`, and network may-write `now`/`digest`/`obs`) carry `mcp:open-world=true` (and read-only when they do not write the store); `teach`, `teach-playbook`, `playbook amend`, `learnings confirm`, `teach-pattern`, and `teach-lookup` carry `mcp:local-write=true` (writes land only in the CLI's own local store); `learnings forget` and `learnings reject` keep honest may-write/destructive defaults.
...
Disable the loop with `--no-learn` per-invocation or `PAGASA_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
